...
| Phase | Développement MVP — curriculum local & bibliothèque (MVP-003) |
| Statut global | Conception close ; Runtime 20/20 ; App Shell + Design System + curriculum local / bibliothèque des séances ; **F-013** en développement (fondation) ; **aucune** `F-xxx` terminée |
...
| Développement | **En cours** | Socle `web/` + Design System + curriculum local / bibliothèque séances ; ticket MVP-003 |
| Tests | **En cours** | Vitest : 6 tests unitaires curriculum reader OK ; pas de campagne produit complète (`09`) |
...
| Cohérence conception ↔ runtime | Sync post-Freeze documentaire effectuée ; runtime applicatif partiel (shell, DS, curriculum local) |
...
| --- | --- |
| Tickets ouverts | 0 |
| Tickets terminés | 3 (MVP-001, MVP-002, MVP-003) |
| Tickets bloqués | 0 |
| Modules applicatifs commencés | App Shell + Design System UI + curriculum local (`web/`) |
...
| Changements Runtime (`17`) | **3** `CH-001`…`CH-003` (MVP-001…MVP-003) |
...
| Domaine | État réel |
| --- | --- |
| Architecture | Spécifiée ; registre `01` ; Frontend **en cours** (shell + DS + curriculum local) ; reste non implémenté |
...
| `00_PROJECT_STATUS.md` | Tableau de bord | **ACTIF** | 5 août 2026 — MVP-003 |
| `01_ARCHITECTURE_STATUS.md` | Architecture réelle | **ACTIF** | 5 août 2026 — MVP-003 |
| `02_FEATURE_STATUS.md` | Features réelles | **ACTIF** | 5 août 2026 — MVP-003 |
...
| `09_TEST_STATUS.md` | Tests réels | **ACTIF** | 5 août 2026 — MVP-003 |
...
| `17_CHANGE_HISTORY.md` | Historique changements | **ACTIF** | 5 août 2026 — CH-003 |
...
1. Ouvrir le prochain ticket MVP (pratique guidée / démarrage séance — hors périmètre MVP-003).  
